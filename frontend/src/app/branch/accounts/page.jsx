...
export default function Page() {
  ...
  const filteredAccounts = useMemo(() => {
    ...
    return accounts.filter(acc =>
      ...
      (getEmployeeName(acc.MaNV) || "").toLowerCase().includes(lower)
    );
  }, [accounts, searchTerm, employees]);
                ...
                <tbody>
                  {filteredAccounts.map(acc => (
                    <tr key={acc.TenDangNhap}>
                      ...
                      <td>
                        <span className={`px-2 py-0.5 rounded text-xs font-semibold ${acc.Quyen === "ADMIN_TOAN_BO" ? "bg-purple-100 text-purple-700" :
